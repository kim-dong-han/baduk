...
## 최근 완료 (최신순, 5건 유지)
- **로그인/로그아웃 UI 리디자인(실제 웹사이트풍 사용자 메뉴)**: 딱딱하던 topnav "로그아웃" 버튼을 **아바타+이름+드롭다운** 계정 메뉴로 교체(`<details class="user-menu">` 무JS, common.css). 드롭다운=아바타·이름·이메일 헤더+🚪로그아웃(POST /logout). 로그인 사용자 정보는 `controller/GlobalUserAdvice`(@ControllerAdvice @ModelAttribute)가 `currentUserName/Email/Picture/Initial`로 전 뷰에 주입 — OAuth2User는 name/email/picture 속성, 데모는 표시명·이니셜 원형 아바타. 7개 템플릿 topnav 반영. login.html도 부드럽게(그라데이션 배경/버튼, 라운드·소프트 섀도). 검증(브라우저): 데모 로그인(바둑러)→아바타 원형+드롭다운(이름/로그아웃) 정상, 로그아웃→/login?logout. ※Java(어드바이스) 변경→재시작, static CSS는 하드리로드 필요할 수 있음
- **사활 문제 전면 상향 교체(101books 고전 사활)**: 기존 9×9 소반 잡기 60개(너무 쉬움)를 **101books.github.io 공개 SGF**(퍼블릭 도메인 고전)로 75개 교체. 책=난이도: 쉬움=현현기경(2단)/보통=하시모토 명작·마에다(단급)/어려움=발양론(7단), 각 25개. 전부 19×19 실전형. ETL(일회성 bash): git tree·contents API로 책별 목록→shuf 샘플→raw 다운로드, 루트노드에 PL[색]·C[프롬프트] 주입(파서가 SZ 없으면 19·PL 없으면 첫 수 색·메인라인=정해로 처리). 책 정해 신뢰라 KataGo 검증 생략. 검증: /api/tsumego/count=75, 난이도별 random 정상, 브라우저 대기화면서 쉬움·어려움 렌더 확인. ※resources 변경 → 재시작 필요(TsumegoService @PostConstruct 로드)

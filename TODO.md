...
## 최근 완료 (최신순, 5건 유지)
- **복기 결과판 놓아보기 + 최선수 힌트**: result.html 클라이언트 전용. ①놓아보기(btnTry) — 현재 국면 위에 캔버스 클릭으로 흑·백 교대 착점(기존 getGroup/hasLiberty로 따냄·자살수 금지), 툴바(되돌리기/처음으로/나가기)·카운터("놓아보기·N수(다음:색)")·Esc종료. tryMode 중 hover/화살표 비활성. ②최선수 힌트(btnHint, 재대국 버튼 옆 노랑) — 현재 보이는 국면의 '다음 최선수'(moves[base+1].candidates[0]/bestMove, 폴백 moves[base])를 바둑판 금색 ★ + 라벨(예: "흑의 최선수는 Q19…예상 형세 흑 +7.5집")로. 놓아보기·일반복기 양쪽 동작. 브라우저 검증 완료. ※bootRun은 build/resources/main 템플릿을 서빙 → 편집 후 src→build 복사해야 반영(캐시 off라 재시작 불필요)
